		...
		try:
			...
			for lesson in lessons:
				...
				if first_date.isoweekday() <= lesson.day:  # если первое занятие попадает в первую неделю семестра
					week_in_timetable, created = WeekInTimetable.objects.get_or_create(week_number=first_week_number,
																					   timetable=timetable)
					day_in_timetable, created = DayInTimetable.objects.get_or_create(date=day, weekday=lesson.day,
																					 week=week_in_timetable)
					lsn = LessonInTimeTable.objects.create(lesson=lesson,
														   start=datetime.datetime.combine(day, start_and_end[0]),
														   end=datetime.datetime.combine(day, start_and_end[1]),
														   day=day_in_timetable)
					print(lsn.start)
					print(lsn.end)
				for week in range(first_week_number + 1, end_week_number):
					day += datetime.timedelta(days=7)
					week_in_timetable, created = WeekInTimetable.objects.get_or_create(week_number=week,
																					   timetable=timetable)
					day_in_timetable, created = DayInTimetable.objects.get_or_create(date=day, weekday=lesson.day,
																					 week=week_in_timetable)
					LessonInTimeTable.objects.create(lesson=lesson,
													 start=datetime.datetime.combine(day, start_and_end[0]),
													 end=datetime.datetime.combine(day, start_and_end[1]),
													 day=day_in_timetable)
				if end_date.isoweekday() >= lesson.day:  # если последнее занятие попадает в последнюю неделю семестра
					...
					week_in_timetable, created = WeekInTimetable.objects.get_or_create(week_number=end_week_number,
																					   timetable=timetable)
					day_in_timetable, created = DayInTimetable.objects.get_or_create(date=day, weekday=lesson.day,
																					 week=week_in_timetable)
					LessonInTimeTable.objects.create(lesson=lesson,
