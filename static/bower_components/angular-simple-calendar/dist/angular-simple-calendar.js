angular.module('500tech.simple-calendar', []).directive('simpleCalendar', function () {
  return {
    ...
    templateUrl: '/static/templates/static/calendar.html',
    controller: ['$scope', function ($scope) {
      ...
      calculateWeeks = function () {
        ...
        for (var day = 1; day < daysInCurrentMonth + 1; day += 1) {
          ...
          if (allowedDate(week[dayNumber])) {
            ...
            week[dayNumber].is_weekend = !!((dayNumber === 0) || (dayNumber === 6));
      ...
      calculateSelectedDate = function () {
        ...
        $scope.selectedDay   = $scope.options._defaultDate.getDate();
          ...
          $scope.prevMonthItem = getPrevMonthItem();
          $scope.nextMonthItem = getNextMonthItem();
      ...
      $scope.nextMonth = function () {
        ...
        function getNextMonthItem(){
            var currIndex = MONTHS.indexOf($scope.selectedMonth);
            if (currIndex === 11) {
                return MONTHS[0];
            } else {
                return MONTHS[currIndex + 1];
            }
        }

        function getPrevMonthItem(){
            var currIndex = MONTHS.indexOf($scope.selectedMonth);
            if (currIndex === 0) {
                return MONTHS[11];
            } else {
